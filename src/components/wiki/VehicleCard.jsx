...
const VehicleCard = ({ item, index, onClick }) => {
    ...
    const getRarityStyles = (rarityName) => {
        ...
        return styles[rarityName] || styles['Rare'];
    };

    const rarity = item.rarity?.name || 'Rare';
    const style = getRarityStyles(rarity);

    const primaryStat = { label: 'Speed', value: item.speed || 0 };
    const secondaryStat = { label: 'Slots', value: item.inventory_slots || 0 };
                    ...
                    <div className="px-8 pb-8 mt-auto space-y-6 relative z-10">
                        <div className="flex items-center gap-3">
                            <div className={`px-3 py-1 rounded-full border border-white/5 bg-black/60 shadow-inner flex items-center gap-2`}>
                                ...
                                <span className={`text-[9px] font-black uppercase tracking-[0.2em] ${style.text}`}>{item.subcategory?.name || 'VEHICLE'}</span>
                            </div>
                        </div>
                            ...
                            <div className="grid grid-cols-2 gap-6 pt-6 border-t border-white/5 mt-auto">
                                <div className="space-y-2">
                                    <div className="flex justify-between items-center px-1">
                                        ...
                                        <span className="text-[11px] font-bold text-white/80">{primaryStat.value} KM/H</span>
                                    </div>
                                    <div className="h-1 w-full bg-white/5 rounded-full overflow-hidden">
                                        <motion.div 
                                            initial={{ width: 0 }}
                                            animate={{ width: `${Math.min((primaryStat.value / 200) * 100, 100)}%` }}
                                ...
                                <div className="space-y-2">
                                    ...
                                    <div className="h-1 w-full bg-white/5 rounded-full overflow-hidden">
                                        <motion.div 
                                            initial={{ width: 0 }}
                                            animate={{ width: `${Math.min((secondaryStat.value / 1000) * 100, 100)}%` }}
                                            transition={{ duration: 1, ease: "easeOut" }}
