...
const BasebuildingCard = ({ item, index, onClick, showDeployed }) => {
    ...
    const Icon = item.subcategory?.name?.toLowerCase().includes('storage') ? Box : Hammer;
    ...
    const primaryStat = { label: 'Value', value: item.sell_price ? `$${item.sell_price}` : 'N/A' };
    const secondaryStat = { label: 'Storage', value: item.slots || 0 };
                            ...
                            <div className="grid grid-cols-2 gap-6 pt-6 border-t border-white/5 mt-auto">
                                <div className="space-y-2">
                                    ...
                                    <div className="h-1 w-full bg-white/5 rounded-full overflow-hidden">
                                        <motion.div 
                                            initial={{ width: 0 }}
                                            animate={{ width: `${Math.min((item.sell_price / 5000) * 100, 100)}%` }}
                                ...
                                <div className="space-y-2">
                                    ...
                                    <div className="h-1 w-full bg-white/5 rounded-full overflow-hidden">
                                        <motion.div 
                                            initial={{ width: 0 }}
                                            animate={{ width: `${Math.min((secondaryStat.value / 100) * 100, 100)}%` }}
                                            transition={{ duration: 1, ease: "easeOut" }}
