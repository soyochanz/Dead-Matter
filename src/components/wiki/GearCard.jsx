...
const GearCard = ({ gear, index, onClick }) => {
    ...
    const rarity = gear.rarity?.name || 'Common';
    const style = getRarityStyles(rarity);

    const getPrimaryStat = () => {
        if (gear.armor_rating > 0) return { label: 'Armor', value: gear.armor_rating };
        if (gear.inventory_slots > 0) return { label: 'Slots', value: gear.inventory_slots };
        return { label: 'Weight', value: gear.weight || 0 };
    };

    const getSecondaryStat = () => {
        if (gear.blunt_protection > 0) return { label: 'Blunt', value: gear.blunt_protection };
        if (gear.insulation > 0) return { label: 'Warmth', value: gear.insulation };
        return { label: 'Durability', value: gear.durability || 100 };
    };

    const primaryStat = getPrimaryStat();
    const secondaryStat = getSecondaryStat();
                            ...
                            <div className="grid grid-cols-2 gap-6 pt-6 border-t border-white/5 mt-auto">
                                <div className="space-y-2">
                                    <div className="flex justify-between items-center px-1">
                                        <span className="text-[8px] font-black text-white/20 uppercase tracking-widest">{primaryStat.label}</span>
                                        <span className="text-[11px] font-bold text-white/80">{primaryStat.value}</span>
                                    </div>
                                    <div className="h-1 w-full bg-white/5 rounded-full overflow-hidden">
                                        <motion.div 
                                            initial={{ width: 0 }}
                                            animate={{ width: `${Math.min(primaryStat.value || 0, 100)}%` }}
                                ...
                                <div className="space-y-2">
                                    <div className="flex justify-between items-center px-1">
                                        <span className="text-[8px] font-black text-white/20 uppercase tracking-widest">{secondaryStat.label}</span>
                                        <span className="text-[11px] font-bold text-white/80">{secondaryStat.value}</span>
                                    </div>
                                    <div className="h-1 w-full bg-white/5 rounded-full overflow-hidden">
                                        <motion.div 
                                            initial={{ width: 0 }}
                                            animate={{ width: `${Math.min(secondaryStat.value || 0, 100)}%` }}
                                            transition={{ duration: 1, ease: "easeOut" }}
